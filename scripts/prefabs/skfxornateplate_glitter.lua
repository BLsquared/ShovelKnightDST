...
local function fn()
	...
	emitter:SetMaxNumParticles(0) --old 12
	...
	local sphere_emitter = CreateSphereEmitter(0.3)
	...
	local function emit_fn()
		...
		px = px - 0.0
		py = py + 0.3 -- the 0.2 is to offset the flame particles upwards a bit so they can be used on a torch
...
return Prefab("common/fx/skfxornateplate_glitter", fn, assets)